...
c~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c
c    The Fortran version of BOBYQA is attached. Its purpose is to seek
c the least value of a function F of several variables, when derivatives
c are not available, where F is specified by the user through a subroutine
c called CALFUN. The name BOBYQA denotes Bound Approximation BY Quadratic
c Approximation, the constraints being lower and upper bounds on every
c variable, which can be set to huge values for unconstrained variables.
c The algorithm is intended to change the variables to values that are close
c to a local minimum of F. The user, however, should assume responsibility for
c finding out if the calculations are satisfactory, by considering carefully
c the values of F that occur. The BOBYQA software has been developed from the
c method of the paper "The NEWUOA software for unconstrained minimization
c without derivatives", in Large-Scale Nonlinear Optimization, editors G. Di
c Pillo and M. Roma, Springer (2006), pages 255-297. A report that describes
c the details of the development is going to be written soon.

c    The attachments in sequence are a suitable Makefile, followed by a main
c program and a CALFUN routine for the "Invdist2" problem, in order to provide
c an example for testing. Then BOBYQA and its six auxiliary routines, namely
c BOBYQB, ALTMOV, PRELIM, RESCUE, TRSBOX and UPDATE, are given. Finally, the
c computed output that the author obtained for the Invdist2 problems is listed.

c    In addition to providing CALFUN, an initial vector of variables and
c the lower and upper bounds, the user has to set the values of the parameters
c RHOBEG, RHOEND and NPT. After scaling the individual variables if necessary,
c so that the magnitudes of their expected changes are similar, RHOBEG is the
c initial steplength for changes to the variables, a reasonable choice being
c the mesh size of a coarse grid search. Further, RHOEND should be suitable for
c a search on a very fine grid. Typically, the software calculates a vector
c of variables that is within distance 10*RHOEND of a local minimum. Another
c consideration is that every trial vector of variables is forced to satisfy
c the lower and upper bounds, but there has to be room to make a search in all
c directions. Therefore an error return occurs if the difference between the
c bounds on any variable is less than 2*RHOBEG. The parameter NPT specifies
c the number of interpolation conditions on each quadratic model, the value
c NPT=2*N+1 being recommended for a start, where N is the number of variables.
c It is often worthwhile to try other choices too, but much larger values tend
c to be inefficient, because the amount of routine work of each iteration is
c of magnitude NPT**2, and because the achievement of adequate accuracy in some
c matrix calculations becomes more difficult. Some excellent numerical results
c have been found in the case NPT=N+6 even with more than 100 variables.

c    The way of calling BOBYQA should be clear from the Invdist2 examples
c and from the comments near the beginning of SUBROUTINE BOBYQA. There are no
c restrictions on or charges for the use of the software. I hope that the time
c and effort I have spent on developing the package will be helpful to much
c research and to many applications.

c January 5th, 2009                    M.J.D. Powell (mjdp@cam.ac.uk)



c %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Makefile %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

c main: altmov.o bobyqa.o bobyqb.o calfun.o main.o prelim.o rescue.o
c trsbox.o update.o
c       ifort -g altmov.o bobyqa.o bobyqb.o calfun.o main.o prelim.o
c rescue.o trsbox.o update.o
c altmov.o: altmov.f
c       ifort -g  -c altmov.f
c bobyqa.o: bobyqa.f
c        ifort -g  -c bobyqa.f
c bobyqb.o: bobyqb.f
c       ifort -g  -c bobyqb.f
c calfun.o: calfun.f
c        ifort -g  -c calfun.f
c main.o: main.f
c       ifort -g  -c main.f
c cprelim.o: prelim.f
c       ifort -g  -c prelim.f
c rescue.o: rescue.f
c       ifort -g  -c rescue.f
c trsbox.o: trsbox.f
c       ifort -g  -c trsbox.f
c update.o: update.f
c       ifort -g  -c update.f
...
C
      ...
      integer nexp,maxe
      parameter (maxe = 100)
      character *200 filename(maxe) 
      integer i,j,m(maxe),n,maxfun,iprint,maxm,maxn,npt,ipowell,iexp,mf
      parameter (maxm=500,maxn=100)
      double precision a(maxn),xl(maxn),xu(maxn),rhobeg,rhoend,w(50000),
     *x(maxm,maxe),yexp(maxm,maxe),ycal(maxm,maxe),eps(maxe),x0(maxe),
     *dsol(maxe),dens(maxe),q(maxe),hb(maxe),db(maxe),tcer(maxe),
     *tot(maxe),epsp(maxe),cer(maxe),solub(maxe),beta(maxe),dp(maxe),
     *f,r(maxm),af(maxn),yf(maxm,maxe),visc(maxe),dab(maxe),kya(maxe),
     *temp(maxe),xk(maxe),z,r2(maxm)
      double precision drand,seed,fant
      common/exp/x,yexp,ycal,eps,x0,dsol,dens,q,hb,db,dp,tot,epsp,cer,
     *solub,beta,visc,dab,kya,temp,xk,m,nexp
...
c	  open(10,file='bobyqa.out')


c	Dados experimentais

c	write(*,*) ' Qual o arquivo de dados experimentais?'
c     read(*,*) filename

      nexp = 1 !COLOCAR NéMERO DE CURVAS QUE SERÇO MODELADAS
      
      !COLOCAR NOMES DOS ARQUIVOS DAS CURVAS
      
      filename(1) = "mateus1.txt"
      filename(2) = "2.txt"
      filename(3) = "3.txt"
      filename(4) = "4.txt"
      filename(5) = "5.txt"
      ...
      open(20,file='saida.txt') !COLOQUE AQUI O NOME DO ARQUIVO DE SAÖDA
      open(25,file='parametros.txt')

      do iexp = 1,nexp
      
      open(10,file=filename(iexp),status='old')

      !ABAIXO, OS DADOS QUE DEVEM ESTAR NO ARQUIVO DA CURVA

      read(10,*) temp(iexp) !temperatura
      read(10,*) eps(iexp) !porosidade do leito
      read(10,*) x0(iexp)   !rendimento global (total extraível)
      read(10,*) dsol(iexp) !densidade do sólido
      read(10,*) dens(iexp) !densidade do solvente
      read(10,*) q(iexp) !vazão de solvente
      read(10,*) hb(iexp) !altura do leito
      read(10,*) db(iexp) !diâmetro do leito
      read(10,*) dp(iexp) !diâmetro da partícula (não é usado)
      read(10,*) tot(iexp) !massa de sólido
      read(10,*) epsp(iexp) !porosidade da partícula (não é usado)
      read(10,*) cer(iexp) !taxa de extração CER (não é usado)
      read(10,*) solub(iexp) !solubilidade
      read(10,*) beta(iexp) !não é usado
      read(10,*) visc(iexp) !viscosidade do solvente
      read(10,*) dab(iexp) !difusividade do soluto no solvente
      read(10,*) m(iexp) !número de pontos experimentais
      
c     Mudança de unidades - de g,cm,min para kg,m,s (SI) 
      
      dsol(iexp) = dsol(iexp)*1000.
      dens(iexp) = dens(iexp)*1000.
      q(iexp) = q(iexp)/(60.*1000.)
      hb(iexp) = hb(iexp)/100.
      db(iexp) = db(iexp)/100.
      dp(iexp) = dp(iexp)/100.
      tot(iexp) = tot(iexp)/1000.
      visc(iexp) = visc(iexp)/1000.
      
      do i=1,m(iexp)
        read(10,*) x(i,iexp),yexp(i,iexp)
        x(i,iexp) = x(i,iexp)*60.
        yexp(i,iexp) = yexp(i,iexp)/1000.
      end do

      close(10)

c      write(20,*)
c      write(*,*) filename(iexp)

      end do
      
           

      n = 2*nexp + 1              ! Número de parâmetros ajustáveis
      npt = 2*n + 1

1     ipowell = ipowell + 1
      fant = f

      !A SEGUIR, DEFINIMOS A FAIXA ONDE DEVEM ESTAR OS PAR¶METROS DO MODELO

      do iexp=1,nexp

        a(2*iexp) = .005*drand(seed)      ! Valor inicial de kxa
        xl(2*iexp) = 0.d0              ! Valor mínimo de kxa
        xu(2*iexp) = .005                ! Valor máximo de kxa
        
        a(2*iexp-1) = .05*drand(seed)   !Valor inicial de kya
        xl(2*iexp-1) = 0.             !Valor m¡nimo de kya
        xu(2*iexp-1) = .05           ! Valor máximo de kya
      
      end do


      a(2*nexp+1) = drand(seed)        !Valor inicial de Xk/X0
      xl(2*nexp+1) = 0.                      !Valor minimo de Xk/X0
      xu(2*nexp+1) = 1.                       !Valor máximo de Xk/X0 (no m ximo 1)

c      do j = 2*nexp+2,n
    
c       a(j) = drand(seed)             
c        xl(j) = 0.d0
c        xu(j) = 1.d0                 !Valores máximos de b, c, d (usados caso se ajuste correla‡Æo para Sh)
...
c     rhobeg = .0001
      ...
      iprint = 1
      maxfun = 5000      ! Numero máximo de iteracoes (em cada rodada)

c      call CALFUN (n,a,f)

      CALL BOBYQA (n,npt,a,xl,xu,rhobeg,rhoend,iprint,maxfun,w)

      call CALFUN (n,a,f)

c	Calculo do valor final da funcao objetivo
      ...
      do iexp=1,nexp
        ...
        do i=1,m(iexp)
          ...
          r(i)=ycal(i,iexp)-yexp(i,iexp)
          r2(i) = r(i)**2.
c          f = f + r(i)**2.              ! Soma de quadrados dos erros
c          f = f + r2(i)
...
c	stop
      ...
      write(*,*)
      write(20,*) 'Powell numero ',ipowell
      write(*,*) 'Powell numero ',ipowell
      write(20,*) 'f = ', f
      ...
      do iexp = 1,nexp
        ...
        do j=1,2
          ...
          write(20,*) 'Curva ',iexp
          write(20,*) 'a(',j,')=',a(2*(iexp-1)+j)

        end do
        
c        write(20,*) 'kya(',iexp,')=',kya(iexp)
      ...
      if (ipowell.lt.1000) go to 1          !Numero de rodadas

        write(20,*) 'Resultado Final'
        write(20,*) 
        write(20,*) 'f =',f
        write(20,*)
c        write(20,*) 'Sh =',af(2*nexp+2),'Re^',af(2*nexp+3),'Sc^(1/3)'
c        write(20,*) 'Sh = ',af(2*nexp+1),'Ra^',af(2*nexp+2), 
c     *  'Re/(Gr^',af(2*nexp+3),'^',af(2*nexp+4)
        
        do iexp=1,nexp
        
          tcer(iexp) = (x0(iexp)-x0(iexp)*af(2*nexp+1))*(1.-eps(iexp))*
     *     dsol(iexp)/(solub(iexp)*af(2*iexp-1)*dens(iexp))

          write(20,*) 'Curva ',iexp
          write(*,*) 'Curva ',iexp
          write(20,*) 'tcer =',tcer(iexp)
          write(*,*) 'tcer =',tcer(iexp)

          
          do j=1,2

            write(20,*) 'a(',j,') =',af(2*(iexp-1)+j)
            write(*,*) 'a(',j,') =',af(2*(iexp-1)+j)

          end do 
          
          write(*,*) 'xk =',af(2*nexp+1)*x0(iexp)
          write(20,*) 'xk =',af(2*nexp+1)*x0(iexp)
          
c          write(20,*) 'kya(',iexp,') =',kya(iexp)
c          write(*,*) 'kya(',iexp,') =',kya(iexp)
...
c	Apresentacao da curva ajustada

          write(20,*) 'Tempo, yexp, ycal'

          do i=1,m(iexp)

            write(20,*) x(i,iexp),yexp(i,iexp),yf(i,iexp)
...
c	FUNCAO DRAND - PARA SORTEAR OS VALORES INICIAIS DOS PARAMETROS
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% calfun.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE CALFUN (n,a,f)

      implicit real*8 (a-h,o-z)
      parameter (maxm=500,maxh=10001,maxe=100)
      dimension a(n),r(maxm)
      integer m(maxe),nexp
      double precision x(maxm,maxe),yexp(maxm,maxe),ycal(maxm,maxe),
     *ynum(2,maxe),xs(maxh,2,maxe),eps(maxe),x0(maxe),dsol(maxe),
     *dens(maxe),q(maxe),hb(maxe),db(maxe),dp(maxe),tot(maxe),cer(maxe),
     *solub(maxe),beta(maxe),tempo(maxe),v(maxe),epsp(maxe),
     *y(maxh,2,maxe),yant(maxh,maxe),jxy(maxh,2,maxe),ermax,
     *visc(maxe),dab(maxe),re(maxe),sc(maxe),kya(maxe),gr(maxe),ra(maxe)
     *,deltadens(maxe),temp(maxe),sh(maxe),kb,xk(maxe),r2(maxm)
      common/exp/x,yexp,ycal,eps,x0,dsol,dens,q,hb,db,dp,tot,epsp,cer,
     *solub,beta,visc,dab,kya,temp,xk,m,nexp
      ...
      pi = 3.1415927
      kb = 1.3806503d-23 !Constante de Boltzmann
      rsol = 1.d-9 !raio da molécula de soluto
      g = 9.81 !Gravidade 
      dsoluto =  895. !densidade do extrato
      
       
c      day = 0.		! Difusividade no CO2 - colocar valor ou zero se desprezar
     
c      icount = icount + 1   ! Contador de iterações
       
      do iexp=1,nexp
      
      tempo(iexp) = x(m(iexp),iexp)
 
      
      eps(iexp) = 1 - 4*tot(iexp)/(pi*(db(iexp)**2)*hb(iexp)*dsol(iexp))
      v(iexp) = 4.d0*q(iexp)/(dens(iexp)*pi*(db(iexp)**2)*eps(iexp)) ! Velocidade do CO2
c      visc(iexp) = 
      dab(iexp) =  kb*temp(iexp)/(6*pi*rsol*visc(iexp)) !Equação de Stokes-Einstein
c      dab(iexp) = 3.3d-9
      
 
      re(iexp) = dens(iexp)*v(iexp)*dp(iexp)/visc(iexp) !Reynolds
      sc(iexp) = visc(iexp)/(dens(iexp)*dab(iexp))      !Schmidt
      deltadens(iexp) = (solub(iexp)*dsoluto + 
     *(1-solub(iexp))*dens(iexp)) - dens(iexp) !Variação de densidade do solvente
      gr(iexp) = (dp(iexp)**3.)*dens(iexp)*g*deltadens(iexp)/
     *(visc(iexp)**2)                                  !Grashof
      ra(iexp) = sc(iexp)*gr(iexp) !Rayleigh
     
      ynum(1,iexp) = 0.

c	Discretizacao da malha (ATENCAO PARA A ESTABILIDADE!!!)

      nh = 5   ! Passos em h
      nt = 2500 ! Passos em t 

      dt = tempo(iexp)/nt   ! Tamanho do passo em t
      dh = hb(iexp)/nh      ! Tamanho do passo em h
      
      xk(iexp) = x0(iexp)*a(2*nexp+1)      
      
c      kya(iexp) = a(3*iexp) !Coeficiente convectivo independente

c      sh(iexp) = (a(2*nexp+1) + 
c    *a(2*nexp+2)*(re(iexp)**a(2*nexp+3))*(sc(iexp)**a(2*nexp+4))) !Correlação com 4 parâmetros
     
c      sh(iexp) = a(2*nexp+2)*
c     *(re(iexp)**a(2*nexp+3))*(sc(iexp)**(1/3))              !Correlação com 2 parâmetros

c      sh(iexp) = a(2*nexp+1)*  
c     *ra(iexp)**(.25)*(re(iexp)/(gr(iexp)**.5))**a(2*nexp+2)  !Correlação com convecção natural (baseada em Lim et al (1989)) - 2 parâmetros
      
c      sh(iexp) = a(2*nexp+1)*  
c     *ra(iexp)**(a(2*nexp+2))*(re(iexp)/(gr(iexp)**a(2*nexp+3)))**
c     *a(2*nexp+4)                                 !Correlação com convecção natural (baseada em Lim et al (1989)) - 4 parâmetros
      
c      kya(iexp) = 6.*(dab(iexp)/(dp(iexp)**2.))*sh(iexp) !Coeficiente convectivo (s^-1)

      sh(iexp) = a(2*iexp-1)*(dp(iexp)**2.)/(6*dab(iexp))
...
c	Condicao Inicial
      ...
      do k=1,nh+1
        ...
        xs(k,1,iexp) = x0(iexp)    ! Concentração inicial no sólido

c		y(k,1) = solub	! Concentração inicial no CO2: 
c		y(k,1) = 0.		! usar zero ou solubilidade
...
c	Condicao de contorno
      ...
      y(1,1,iexp) = 0.          ! Concentração na entrada da coluna
c      y(nh+2,1) = y(nh+1,1)    ! Concentração na saída da coluna (dY/dh = 0)

c	Calculo dos pontos da malha

      do k=1,nh

        if (xs(k,1,iexp).gt.xk(iexp)) then 

          jxy(k,1,iexp) = a(2*iexp-1)*(solub(iexp) - y(k,1,iexp))     !Sovová

c			jxy(k,1) = a(2)*(solub - y(k,1))*dlog(1.d-3)*
c     *		exp(((x0-xs(k,1))/(x0-a(1))))				!Cygnarowicz

        end if

        if (xs(k,1,iexp).le.xk(iexp)) then 

          jxy(k,1,iexp) = a(2*iexp)*xs(k,1,iexp)*(1.d0 - y(k,1,iexp)/
     *    solub(iexp))                                          !Sovová

c			jxy(k,1) = a(3)*(solub - y(k,1))*(xs(k,1)/a(1))		!Lack (adaptado)

c			jxy(k,1) = a(3)*xs(k,1)								!Pekhov
...
c     *		exp(((x0-xs(k,1))/(x0-a(1))))						!Cygnarowicz
        ...
        xs(k,2,iexp) = xs(k,1,iexp) - dt*jxy(k,1,iexp)*dens(iexp)/
     *  (dsol(iexp)*(1.d0-eps(iexp)))

        xs(k,1,iexp) = xs(k,2,iexp)

        yant(k+1,iexp) = y(k+1,1,iexp)

c		SOVOVA NORMAL

        y(k+1,1,iexp) = y(k,1,iexp) + dh*jxy(k,1,iexp)/(eps(iexp)*
     *  v(iexp))
...
c		y(k+1,2) = y(k+1,1) + 
c     *(jxy(k,1)/eps - (v/dh)*(y(k+1,1)-y(k,1)))*dt

c		y(k+1,1) = y(k+1,2)   

c		****************************************

c		SOVOVA COM dY/dt E TERMO DIFUSIVO

c		y(k+1,2) = y(k+1,1) + 
c     *  (jxy(k,1)/eps - (v/dh)*(y(k+1,1)-y(k,1))
c     *  +(a(4)/(dh**2.))*(y(k+2,1)-2*y(k+1,1)+y(k,1)))*dt
...
C     Integração Numérica: PARA SOVOVÁ NORMAL USAR y(nh+1,1). PARA OS DEMAIS USAR y(nh+1,2)

      ynum(2,iexp) = ynum(1,iexp) + dt*(y(nh+1,1,iexp)+yant(nh+1,iexp))*
     *q(iexp)/2.
...
c	Interpolacao
      ...
      do i=1,m(iexp)
        ...
        if (t.ge.x(i,iexp).and.x(i,iexp).ge.(t-dt)) then

           ycal(i,iexp) = ynum(1,iexp) + (ynum(2,iexp)-ynum(1,iexp))*
     *    (x(i,iexp)-t+dt)/dt
      ...
      ynum(1,iexp) = ynum(2,iexp)

      if (t.lt.tempo(iexp))  then 
      
        go to 5

c      else 
      
c      if (iexp.eq.nexp) go to 10
...
c	Apresentacao dos resultados das iteracoes

c	write(*,*)
c	write(*,*) 'it=',icount,' f=',f,' xk=',a(1),' kya=',a(2),' kxa=',
c     *a(3)
c     * ,' Day=',a(4)		! Habilitar se usar Day ajustável
c	write(*,*)
c	write(5,*)	
c	write(5,*) 'it=',icount,' f=',f,' xk=',a(1),' kya=',a(2),' kxa=',
c     *a(3)
c     * ,' Day=',a(4)		! Habilitar se usar Day ajustável
c	write(5,*)
...
c	NUNCA MUDAR NADA DAQUI PRA BAIXO!!!!!!!!!!!!
...
C     XNEW is chosen by SUBROUTINE TRSBOX or ALTMOV. Usually XBASE+XNEW is
cthe
...
C       function subject to the constraints that have been mentioned, its
cmain
...
C     be done. The following cycle through the new interpolation points
cbegins
...
C     The arrays S, HS and HRED are also used for working space. They hold
cthe
C       current search direction, and the changes in the gradient of Q
calong S
...
c    2D output with M =   5,  N =  10  and  NPT =  16

c   New RHO = 1.0000D-02     Number of function values =    36
c   Least value of F =  5.680729791421958D+00         The corresponding X is:
c    2.221147D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   3.265332D-01
c
c   New RHO = 1.0000D-03     Number of function values =    60
c   Least value of F =  5.680354430001144D+00         The corresponding X is:
c    2.603234D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.612788D-01
c
c   New RHO = 1.0000D-04     Number of function values =    73
c  Least value of F =  5.680353929615947D+00         The corresponding X is:
c    2.606974D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.615739D-01
c
c   New RHO = 1.0000D-05     Number of function values =    90
c   Least value of F =  5.680353888326326D+00         The corresponding X is:
c    2.613216D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.611715D-01
c
c   New RHO = 1.0000D-06     Number of function values =   106
c   Least value of F =  5.680353888089773D+00         The corresponding X is:
c    2.612550D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.612414D-01
c
c   At the return from BOBYQA     Number of function values =   126
c   Least value of F =  5.680353888084283D+00         The corresponding X is:
c    2.612470D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.612471D-01
c

c    2D output with M =   5,  N =  10  and  NPT =  21
c
c   New RHO = 1.0000D-02     Number of function values =    44
c   Least value of F =  5.608897863679983D+00         The corresponding X is:
c    9.776088D-01   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.757050D-13
c
c   New RHO = 1.0000D-03     Number of function values =    59
c   Least value of F =  5.601560245973537D+00         The corresponding X is:
c   1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -3.657320D-03

c   New RHO = 1.0000D-04     Number of function values =    73
c   Least value of F =  5.601533978562353D+00         The corresponding X is:
c    1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -5.697377D-05

c   New RHO = 1.0000D-05     Number of function values =    79
c   Least value of F =  5.601533972205489D+00         The corresponding X is:
c    1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -3.112239D-06

c   New RHO = 1.0000D-06     Number of function values =    94
c   Least value of F =  5.601533972205489D+00         The corresponding X is:
c    1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c  -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -3.112239D-06

c   At the return from BOBYQA     Number of function values =   106
c   Least value of F =  5.601533972186465D+00         The corresponding X is:
c    1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   3.401210D-08
c
c
c    2D output with M =  10,  N =  20  and  NPT =  26
c
c   New RHO = 1.0000D-02     Number of function values =    34
c   Least value of F =  3.292325831206315D+01         The corresponding X is:
c    1.000000D+00   8.361927D-01   3.514740D-01   9.987697D-01  -3.514740D-01
c    1.000000D+00  -1.000000D+00   9.333774D-01  -9.994928D-01   8.814432D-02
c   -1.000000D+00  -9.941032D-01  -2.656296D-01  -1.000000D+00   2.687293D-01
c   -1.000000D+00   1.000000D+00  -9.941032D-01   9.994928D-01   8.814432D-02
c
c   New RHO = 1.0000D-03     Number of function values =    79
c   Least value of F =  3.220482042767259D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.536511D-01   1.000000D+00  -3.646051D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00  -1.972700D-02
c   -1.000000D+00  -1.000000D+00  -3.565634D-01  -1.000000D+00   3.628796D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.081275D-02
c
c   New RHO = 1.0000D-04     Number of function values =   122
c   Least value of F =  3.220305488441002D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.619671D-01   1.000000D+00  -3.613015D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00  -1.890679D-04
c   -1.000000D+00  -1.000000D+00  -3.618465D-01  -1.000000D+00   3.615325D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   9.312810D-05
c
c   New RHO = 1.0000D-05     Number of function values =   151
c   Least value of F =  3.220305338970701D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.616087D-01   1.000000D+00  -3.615690D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00  -2.205656D-05
c   -1.000000D+00  -1.000000D+00  -3.616288D-01  -1.000000D+00   3.615716D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   7.437381D-06
c
c   New RHO = 1.0000D-06     Number of function values =   179
c   Least value of F =  3.220305336886207D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.616075D-01   1.000000D+00  -3.616069D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   8.604585D-07
c   -1.000000D+00  -1.000000D+00  -3.616078D-01  -1.000000D+00   3.616084D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.821655D-06
c
c   At the return from BOBYQA     Number of function values =   209
c   Least value of F =  3.220305336883057D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.616078D-01   1.000000D+00  -3.616079D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   1.100284D-07
c   -1.000000D+00  -1.000000D+00  -3.616080D-01  -1.000000D+00   3.616079D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -2.025736D-07
c
c
c    2D output with M =  10,  N =  20  and  NPT =  41
c
c   New RHO = 1.0000D-02     Number of function values =    45
c   Least value of F =  3.221724258591880D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.811180D-01   1.000000D+00  -3.811180D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   1.224647D-16
c   -1.000000D+00  -1.000000D+00  -3.811180D-01  -1.000000D+00   3.811180D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -2.449294D-16
c
c   New RHO = 1.0000D-03     Number of function values =    65
c   Least value of F =  3.220308936260133D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.625827D-01   1.000000D+00  -3.625827D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   1.224647D-16
c   -1.000000D+00  -1.000000D+00  -3.625827D-01  -1.000000D+00   3.625827D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -2.449294D-16
c
c   New RHO = 1.0000D-04     Number of function values =   101
c   Least value of F =  3.220305383315654D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.614491D-01   1.000000D+00  -3.616412D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   2.484750D-04
c   -1.000000D+00  -1.000000D+00  -3.615864D-01  -1.000000D+00   3.616618D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   2.400444D-04
c
c   New RHO = 1.0000D-05     Number of function values =   132
c   Least value of F =  3.220305337838101D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.615994D-01   1.000000D+00  -3.616320D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   4.099658D-06
c   -1.000000D+00  -1.000000D+00  -3.616018D-01  -1.000000D+00   3.616037D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00   4.861099D-05
c
c   New RHO = 1.0000D-06     Number of function values =   165
c   Least value of F =  3.220305336885800D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.616075D-01   1.000000D+00  -3.616092D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   1.515454D-06
c   -1.000000D+00  -1.000000D+00  -3.616073D-01  -1.000000D+00   3.616070D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -3.422797D-07
c
c   At the return from BOBYQA     Number of function values =   182
c   Least value of F =  3.220305336883060D+01         The corresponding X is:
c    1.000000D+00   1.000000D+00   3.616077D-01   1.000000D+00  -3.616078D-01
c    1.000000D+00  -1.000000D+00   1.000000D+00  -1.000000D+00   1.910563D-08
c   -1.000000D+00  -1.000000D+00  -3.616078D-01  -1.000000D+00   3.616080D-01
c   -1.000000D+00   1.000000D+00  -1.000000D+00   1.000000D+00  -1.376918D-07
c
c
